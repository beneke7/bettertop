...
For SSH sessions, run BetterTop inside `tmux` so it survives a dropped
connection; reconnect with the same attach command:

```sh
tmux new -As bettertop
~/.local/bin/bettertop
```

If SSH closes the terminal itself, BetterTop now exits cleanly instead of
spinning in the background.

For a manual Linux x86_64 install, download the [v0.1.8 archive](https://github.com/beneke7/bettertop/releases/download/v0.1.8/bettertop-0.1.8-linux-x86_64.tar.gz),
